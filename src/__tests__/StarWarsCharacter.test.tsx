// src/__tests__/StarWars.test.tsx
...
describe("CharacterData Component", () => {
  ...
  test("displays character details correctly", () => {
    ...
    const characters = screen.getAllByText(/Luke Skywalker/i);
    expect(characters[0]).toBeInTheDocument();

    const height = screen.getAllByText(/172/i);
    expect(height[0]).toBeInTheDocument();

    const mass = screen.getAllByText(/77/i);
    expect(mass[0]).toBeInTheDocument();
  });

  test("handles character click event", () => {
    const store = createTestStore();
